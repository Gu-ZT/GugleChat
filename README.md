...
- **语音频道** — WebRTC P2P 语音通话，星形拓扑主机转发，单人单房间
- **视频通话** — 摄像头支持，双击卡片放大
- **屏幕共享** — 与摄像头互斥，支持预览和放大
- **文件上传** — 拖拽/点击上传，图片内联渲染，视频/音频嵌入式播放
- **XSS 防护** — 前端 markdown-it 禁用 raw HTML + sanitize，后端过滤
- **智能主机选举** — NAT 类型检测 + 带宽评分，自动选最优主机，支持手动 NAT 类型覆盖
- **RNNoise AI 降噪** — 基于 WASM 的实时神经网络降噪，与浏览器降噪互斥可选
- **音频处理管线** — 回声消除 / 噪声抑制 / RNNoise 独立开关，通话中实时生效
- **语音活动检测** — 说话时头像绿圈高亮，P2P 延迟/质量分实时显示
- **麦克风控制** — 静音/取消静音，音量滑条，输入设备切换，通话外也可操作
- **扬声器控制** — 静音/取消静音，音量滑条，输出设备切换
- **自我监听** — 可脱离语音频道独立监听麦克风输入
- **心跳检测** — 客户端离线自动踢出语音房间
- **暗黑模式** — Arco Design CSS 变量主题切换
...
- **NAT 覆盖**：手动选择 NAT 类型，取手动/自动中更差值为准，避免高估
- **单人单房间**：服务端强制阻止用户同时出现在多个语音频道
- **自动切换**：主机离开自动选新主机，心跳超时自动踢出
- **NAT 检测**：多 STUN 服务器检测，区分 Cone NAT / Symmetric NAT
- **TURN 支持**：设置页填写 TURN 服务器穿透对称 NAT
- **RNNoise AI 降噪**：AudioWorklet + WASM，48kHz 实时处理，与浏览器降噪互斥
- **延迟显示**：P2P 直连延迟 + 服务器延迟，卡片右上角实时刷新
...
- **后端地址**：留空使用代理，填入直连
- **主题**：暗黑 / 明亮
- **NAT 类型**：手动覆盖自动检测（Auto / NAT1-4），取更差值
