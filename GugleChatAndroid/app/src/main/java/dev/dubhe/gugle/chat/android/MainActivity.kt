                ...
                Surface(Modifier.fillMaxSize()) {
                    if (loggedIn) {
                        ...
                        RegisterScreen(
                            ...
                            onRegister = { u, e, p, url -> viewModel.register(u, e, p, url) { error = it } },
                            onNavigateLogin = { showRegister = false }, error = error
                        )
                    } else {
                        LoginScreen(
                            initialBackendUrl = savedBackendUrl,
                            onLogin = { u, p, url -> viewModel.login(u, p, url) { error = it } },
                            onNavigateRegister = { showRegister = true }, error = error
                        )
                    }
                }
            }
